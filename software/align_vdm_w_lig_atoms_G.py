...
class VDMCloud:
    def __init__(self,wdir,list_fn,aln_res_name,func_name,target_pdb,target_resis,align_atoms=['N7','C2','C6']):
    ...
    def read_target_coords(self):
        ...
        self.ref_root_R = [root_R['C2'],root_R['C6'],root_R['N7']]
        return
            ...
            with open('%s'%pdbname) as fp:
                for line in fp:
                    if not line.startswith('ATOM') and not line.startswith('HETATM'):
                        continue
                    atm = Atom(line)
                    if line.startswith('ATOM') and 'DG' not in line:
                        all_R.append(atm.coord)
                        template.append(atm.template)
                    elif atm.atmName in self.align_atoms and \
                            'DG' in line: #line.startswith('HETATM'):'
                        root_R[atm.atmName] = atm.coord
#                        all_R.append(atm.coord)
#                    else:
#                        all_R.append(atm.coord)
            #
            root_R = [root_R['C2'],root_R['C6'],root_R['N7']]
